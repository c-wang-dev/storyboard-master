...
【场景提取规则（location / time 务必准确）】
- location 必须是**具体地点**（建筑/地形/室内外），从剧本动作与对白上下文推断；**禁止**输出"未知/未识别/未明确/无/null/场景"这类占位值
- 同一片段地点统一命名（如"屋顶/房顶/屋檐"统一为"屋脊"）
- time 填时间/氛围（如"月圆之夜""深夜""黄昏"），无明确时间时按氛围推断
- 示例：
  - "刀客在青石长街上缓步而行，衣角带霜。" → location="古镇青石长街", time="月圆之夜"
  - "黎三倒挂屋檐，短刃出鞘。两人在屋脊对峙。" → location="屋脊", time="夜晚"
  - "他推开房门，烛火摇曳。" → location="书房", time="夜"
  - "一记对拼，二人分开，各自喘息。"（前文在屋脊打斗）→ location="屋脊", time="夜晚"
